...
--------------------
--------------------
--Vyfiltrování NULLových hodnot ve sloupci pod_kat v tabulce urazy--
--vytvoření nové tabulky z části starší tabulky se dělá pomocí SELECT * INTO <název nové tabulky>
    SELECT
        *
    INTO
        pady
...
ALTER TABLE
    dopravni_urazy
ADD CONSTRAINT
    FK_dopravni_urazy_kraj_pacient__id
FOREIGN KEY
    (kraj_pacient)
REFERENCES
    dim_kraj (id_kraj);
...
ALTER TABLE
    dopravni_urazy
ADD CONSTRAINT
    FK_dopravni_urazy_kraj_icz__id
FOREIGN KEY
    (kraj_icz)
REFERENCES
    dim_kraj (id_kraj);
...
ALTER TABLE
    dopravni_urazy
ADD CONSTRAINT
    FK_dopravni_urazy_typ_urazu__id
FOREIGN KEY
    (pod_kat)
REFERENCES
...
ALTER TABLE
    dopravni_urazy
ADD CONSTRAINT
    FK_dopravni_urazy_tize_urazu__id
FOREIGN KEY
    (vaznost)
REFERENCES
...
ALTER TABLE
    dopravni_urazy
ADD CONSTRAINT
    FK_dopravni_urazy_umrti_po_urazu__id
FOREIGN KEY
    (umrti)
REFERENCES
...
--------------------
--------------------
---Zjištění, proč nejde vytvořit cizí klíč, za pomoci vypsání hodnot, které nemají k sobě protikus---
SELECT
    *
FROM
    dopravni_urazy du
    LEFT JOIN dim_tize_urazu dtu ON du.vaznost = dtu.id_tize_urazu
WHERE
    dtu.id_tize_urazu IS NULL
----cizí klíč se neměl na co navázat

---zjištění unikátních hodonot v obou tabulkách
SELECT DISTINCT vaznost FROM dopravni_urazy
SELECT id_tize_urazu FROM dim_tize_urazu 

---v jedné tabulce byly hodnoty s pomlčkou a v druhé ne
---přeměna pomlčky na nic
---povedeno pomocí tranzice, abych si neumazala data, potvrzeno správné převedení Commitem
BEGIN TRAN
UPDATE dim_tize_urazu SET [id_tize_urazu] = REPLACE([id_tize_urazu], N'-', N'')
COMMIT TRAN
---přeměna stejného problém u druhých dvou tabulek
UPDATE dim_umrti_po_urazu SET [id_umrti_po_urazu] = REPLACE([id_umrti_po_urazu], N'-', N'')